...
float Triangle::getLeft() const     { return pos.x - (size.x * 0.5f / 2); }
float Triangle::getRight() const    { return pos.x + (size.x * 0.8f / 2); }
float Triangle::getTop() const      { return pos.y + (size.y * 0.6f / 2); }
float Triangle::getBottom() const   { return pos.y - (size.y * 0.8f / 2); }

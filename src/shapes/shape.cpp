...
void Shape::setVelocityY(float velocity) { velocityY = velocity; }
void Shape::setIsJumping(bool jumping) { isJumping = jumping; }
...
float Shape::getGravity() const { return gravity; }
bool Shape::getIsJumping() const { return isJumping;}
float Shape::getVelocityY() const { return velocityY; }

// jump function
void Shape::jump(float height) {
    if (!isJumping) {
        velocityY = height;
        isJumping = true;
    }
...
bool Shape::isOverlapping(const vec2 &point) const {
    ...
    // if the point is within the area, overlapping is true, else false
